...
var isValidOperation = function (method, element) {
  ...
  switch (method) {
    case 'GET':
      ...
      validity = element && _(ELEMENTS_GET).include(element.split('?')[0]);
      break;
    case 'POST':
...
var makeUrl = function (pageId, element, identifiers) {
...
StatusPageRequest.prototype.sendRequest = function (method, element, pathparams, args, callback) {
  var self = this,
  ...
  if (isValidOperation(method, element)) {
    ...
    var host = statuspage.host,
        ...
        url = makeUrl(statuspage.pageid, element, pathparams),
        httpVerb = method,
